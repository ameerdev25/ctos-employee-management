...
import EmployeeCardDisplayEdit from "@/components/EmployeeCardDisplayEdit";
  ...
  return (    
    <div className="container mx-auto px-3 py-4 grid grid-cols-3 gap-4 bg-white rounded shadow mt-10">
      {employees.map((employee, i) => (
        <EmployeeCardDisplayEdit key={i} avatar={employee.avatar} email={employee.email} firstName={employee.first_name} lastName={employee.last_name} age={employee.age} salary={employee.salary} id={employee.id}  />
      ))}
    </div>
  );
}
